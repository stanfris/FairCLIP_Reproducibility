...
class fairface_dataset(torch.utils.data.Dataset):
    def __init__(self, dataset_dir='../../data/fairface/', preprocess=None, subset='Training', summarized_notes_file_train='fairface_label_train.csv', summarized_notes_file_val='fairface_label_val.csv', ruleout_unknown=False):
    ...
    def __getitem__(self, idx):
        ...
        slo_fundus = self.preprocess(img)
        row = self.data.loc[idx].to_dict()
        note = "this person is " + row['age'] + " years old, and their race is: " + row['race']
